			...
			<div class="col-lg-6">
				<form action="" method="post">
					<div class="input-group">
						<input type="text" class="form-control bg-light border-0 small" placeholder="Enter Instagram Post URL..." aria-label="Search" aria-describedby="basic-addon2" name="id" autocomplete="off" value="<?= $this->input->post('id'); ?>">
		...
		<div class="row mt-5">
			<?php if($this->input->post('submit')): ?>
				<?php if($response['status'] == 200): ?>
					<?php if (key_exists('video_url', $response)): ?>
						<?php foreach ($response['video_url'] as $result): ?>		
							...
							<?php else: ?>
								<div class="col-md-4 mb-3">
									<div class="card">
										<video src="<?= $response['first_video'];?>" class="card-img-top video-custom" controls>
										</div>
										<div class="card-body text-center">
											<a href="<?= $response['first_video'];?> . '&dl=1'; ?>" class="btn btn-success" download><i class="fas fa-download"></i> Download</a>
										</div>
									</div>
								<?php endif; ?>
								<?php else: ?>
						...
						<div class="row mt-5">
							<div class="col ml-3 border-left-success h-100 py-2">
								<h5>How To Download Instagram Videos?</h5>
								<ul>
